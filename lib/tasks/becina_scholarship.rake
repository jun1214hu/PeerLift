...
namespace :import do
  desc "imports det scholarship data from scholarship csv"
  task :becina_scholarships => :environment do
    require 'csv'
    CSV.foreach('db/detscholarship.csv', :headers => true) do |row|
      program_name = row[0]
      image_url = row[1]
      ...
      program_url = row[5]
      description_short = row[6]
      description_long = row[7]
      app_count = row[8]
      essay_count = row[9]
      transcript_count = row[10]
      resume_count = row[11]
